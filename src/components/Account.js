...
import { Context, useGlobalState } from '../Context';
...
const AccountButton = styled.div`
  ...
  background: ${({ authenticate }) =>
    authenticate
      ? 'linear-gradient(174deg, rgba(64,213,55,1) 0%, rgba(7,255,0,1) 80%, rgba(0,255,126,1) 100%)'
      : '#fff'};
  cursor: pointer;
  margin: 10px 0px;
`;
...
const Text = styled.p`
  margin: auto 0;
  color: ${({ authenticate }) => (authenticate ? '#fff' : '#08e41a')};
  font-weight: 600;
  font-size: 16px;
`;
